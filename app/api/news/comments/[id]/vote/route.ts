...
async function authorize(request: Request, id: string) {
  ...
  const exists = await prisma.newsComment.count({ where: { id, post: { isPublished: true } } });
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await prisma.newsCommentVote.upsert({
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const auth = await authorize(request, id);
  if ("error" in auth) return auth.error;
  await prisma.newsCommentVote.deleteMany({ where: { commentId: id, userId: auth.user.id } });
