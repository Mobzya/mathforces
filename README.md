...
Компьютер может быть подключён к роутеру по Ethernet, а телефон — по Wi-Fi: Wi-Fi на самом
компьютере не нужен. Для устройств из другой сети или проверки через мобильный интернет запустите
приложение командой `npm run dev`, а во втором терминале — `npm run tunnel`. Команда напечатает
временный HTTPS-адрес `trycloudflare.com`; используйте его только с тестовыми данными и остановите
после проверки через `Ctrl+C`.
...
| `npm run tunnel`         | временный публичный HTTPS-туннель к порту 3000  |
